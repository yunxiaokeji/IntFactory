define(function (require, exports, module) {
    ...
    ObjectJS.init = function () {
        var _self = this;
        _self.clientsChart = ec.init(document.getElementById('kanbanRPT'));
        _self.bindEvent();
    }
    ObjectJS.bindEvent = function () {
        ...
        $(".kanban-box li").click(function () {
            var _this = $(this);
            if (!_this.hasClass("hover")) {
                _this.siblings().removeClass("hover");
                _this.addClass("hover");
                Params.itemType = _this.attr("id");
                _self.sourceDate();
            }
        });
        _self.getReportData();
    ...
    //获取统计
    ObjectJS.getReportData = function () {
        if (Params.dateType == 1) {
            $(".kanban-box .today span").html("今天：");
            $(".kanban-box .last span").html("昨天：");
        } else if (Params.dateType == 2) {
            $(".kanban-box .today span").html("本周：");
            $(".kanban-box .last span").html("上周：");
        } else if (Params.dateType == 3) {
            $(".kanban-box .today span").html("本月：");
            $(".kanban-box .last span").html("上月：");
        }
        Global.post("/Report/GetKanBanReport", Params, function (data) {
            for (var i = 0; i < data.items.length; i++) {
                var item = data.items[i];
                $("#" + item.DataType).find("h2").html(item.Total);
                $("#" + item.DataType).find(".today label").text(item.Today);
                $("#" + item.DataType).find(".last label").text(item.LastDay);
            }
        });
    }
    //按时间周期
    ObjectJS.sourceDate = function () {
        ...
        Global.post("/Report/GetKanBanItemReport", Params, function (data) {
                    ...
                    feature: {
                        dataView: {
                            ...
                            optionToContent: function (opt) {
                                ...
                                var table = '<table class="table-list"><tr class="tr-header"><td>日期</td> <td>值</td></tr>';
