...
define(function (require, exports, module) {
    require("plug/qiniustorage/plupload/plupload.full.min.js");
    ...
    function QiniuJsSDK() {
        ...
        this.uploader = function (op) {
            var defaults = {
                runtimes: 'html5,flash,html4', // �ϴ�ģʽ,������?
                browse_button: '', // �ϴ�ѡ��ĵ�ѡ��ť��**����**
                container: '',// �ϴ����� DOM ID��Ĭ���� browser_but
                drop_element: '',// ��ҷ�ϴ�����Ԫ��?ID����ҷ�ļ����ļ�
                flash_swf_url: '/modules/plug/qiniustorage/plupload/Moxie.swf',
                file_path: "/Content/UploadFiles/",//���� flash,���·��

                get_new_uptoken: false, // �����ϴ��ļ���ʱ���Ƿ�ÿ�ζ����»�ȡ�µ� uptoken
                domain: "zngc-intfactory", // bucket ������������Դʱ�õ�?*����*
                uptoken_url: "/Plug/GetToken",// Ajax ���� uptoken ?Url?*ǿ�ҽ�������**��������ṩ?

                max_file_size: '5mb', // ����ļ������?
                chunk_size: '5mb', // �ֿ��ϴ�ʱ��ÿ�����?

                dragdrop: true, // �������ҷ�ϴ�
                auto_start: true, // ѡ���ļ����Զ��ϴ�����ر���Ҫ�Լ����¼�������?
                multi_selection: true,//֧�ֶ�?
                unique_names: false, // Ĭ�� false��key Ϊ�ļ�����������ѡ�JS-SDK ��Ϊÿ���ļ��Զ�����key���ļ���?
                url: qiniuUploadUrl,
                multipart_params: {
                    token: ''
                }
            ...
            var getFileKey = function (up, file, func) {
                ...
                var key = up.getOption("file_path") + (date.getFullYear() + (date.getMonth() + 1) + date.getDate() + "/") + date.valueOf() + "." + fileExtension;
            ...
            uploader.bind('FilesAdded', function (up, files) {
                ...
                that.FilesAddedCallBack(up, files);
            });
            ...
            uploader.bind('UploadProgress', function (up, file) {
                ...
                that.UploadProgressCallBack(up, file);
            });
                    ...
                    if (file) {
                        switch (err.code) {
                            ...
                            case plupload.FILE_SIZE_ERROR:
                                ...
                                errTip = '����������ϴ�' + max_file_size + '�������ļ���ʹ�������й��ߡ�';
                                break;
            ...
            uploader.bind('FileUploaded', (function (_FileUploaded_Handler) {
                return function (up, file, info) {
                    ...
                    var FileUploadedCallBack = function (up, file, info) {
                        var fileItem = $("#li_" + file.id);
                        var info = JSON.parse(info);

                        fileItem.find('.progress-number').remove();
                        fileItem.find('.progress-file').remove();
                        fileItem.find('.file-size').show();

                        fileItem.find('.delete').data('key', info.key);
                        fileItem.data({
                            'filepath': '',
                            'filename': info.key,
                            'filesize': file.size,
                        ...
                        for (var j = 0; j < pictypes.length; j++) {
                            if (pictypes[j] == fileExtension) {
                                var src = 'http://o9h6bx3r4.bkt.clouddn.com/' + info.key;
                                var imageObj = $('<img src="' + src + '" />');
                                fileItem.prepend(imageObj);
                                break;
                            }
                        }

                        fileItem.find(".delete").click(function () {
                            fileItem.remove();
                    ...
                    FileUploadedCallBack(up, file, info.response);
        ...
        this.FilesAddedCallBack = function (up, files) {
            var imgs = [];
            var attachments = [];
            var pictypes = ["jpg", "png", "jpeg", "x-png", "x-tiff", "x-pjpeg"];
            for (var i = 0; i < files.length; i++) {
            ...
            var fileBox = $("#" + up.getOption("file_container"));
            ...
            if (imgs.length > 0) {
                ...
                doT.exec(templateUrl, function (template) {
                    ...
                    InnerHtml = $(InnerHtml);
                    $("#" + up.getOption("picture_container")).append(InnerHtml).fadeIn(300);
                });
            }
        }

        this.UploadProgressCallBack = function (up, file) {
