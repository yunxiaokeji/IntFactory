...
namespace IntFactoryBusiness
{
    public class OrdersBusiness
    {
        ...
        public List<OrderEntity> GetOrders(EnumOrderSearchType searchOrderType, EnumSearchType searchtype, string entrustType, string typeid, int status, EnumOrderSourceType sourceType, int orderStatus,int publicStatus, int mark,
                                                ...
                                                string begintime, string endtime, string keyWords, string orderBy, int pageSize, int pageIndex, ref int totalCount, ref int pageCount, string userid, string clientid)
        {
            ...
            DataSet ds = OrdersDAL.BaseProvider.GetOrders((int)searchOrderType, (int)searchtype, entrustType, typeid, status, (int)sourceType, orderStatus,publicStatus, mark, paystatus, warningstatus, 
        ...
        public List<OrderEntity> GetOrders(string keyWords,string categoryid,int pageSize, int pageIndex, ref int totalCount, ref int pageCount, string clientid)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                OrderHelper.HandleEntrustOrder(model, string.Empty, clientid);
        ...
        public OrderEntity GetOrderByID(string orderid, string clientid)
        {
            ...
            if (ds.Tables["Order"].Rows.Count > 0)
            {
                ...
                var tempClientid = string.IsNullOrEmpty(model.EntrustClientID) ? model.ClientID : model.EntrustClientID;
                ...
                model.OrderProcess = SystemBusiness.BaseBusiness.GetOrderProcessByID(model.ProcessID, tempClientid);
                ...
                if (model.Status > 0 && ds.Tables["Tasks"].Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables["Tasks"].Rows)
                    {
                        ...
                        task.FillData(dr);
                        task.Owner = OrganizationBusiness.GetUserCacheByUserID(task.OwnerID, tempClientid);
                        model.Tasts.Add(task);
                    }
                }
                else
                {
                    model.OrderProcess.OrderStages = SystemBusiness.BaseBusiness.GetOrderStages(model.ProcessID, tempClientid);
                }
                ...
                model.OrderMember = new List<OrderMemberEntity>();
                foreach (DataRow dr in ds.Tables["Members"].Rows)
                {
                    OrderMemberEntity item = new OrderMemberEntity();
                    item.FillData(dr);
        ...
        public string CreateOrderGoodsDoc(string orderid, string taskid, EnumGoodsDocType type, int isover, string expressid, string expresscode, string details, string remark, string ownerid, 
                                          string operateid, string clientid, string jsonParas = "", string othersysid = "", string processid = "")
        {
            var dal = new OrdersDAL();
            string id = Guid.NewGuid().ToString().ToLower();

            bool bl = dal.CreateOrderGoodsDoc(id, orderid, taskid, (int)type, processid, isover, expressid, expresscode, details, remark, ownerid, operateid, clientid);
            if (bl)
            {
        ...
        public bool AddOrderMembers(string orderid, string userid, string operateid, string ip, string clientid, out int result)
        {
            bool flag = OrdersDAL.BaseProvider.AddOrderMembers(orderid, userid, operateid, clientid, out result);
        ...
        public bool RemoveOrderMember(string orderid, string userid, string operateid, string ip, string clientid)
        {
            bool flag = OrdersDAL.BaseProvider.RemoveOrderMember(orderid, userid);
